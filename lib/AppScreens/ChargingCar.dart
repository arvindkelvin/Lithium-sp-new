...
class _ChargingCarState extends State<ChargingCar>
  ...
  //var image7 = new Regula.MatchFacesImage();
  ...
  Future<void> saveDraft() async {
    ...
    await prefs.setBool('showdropRow', showdropRow);
    ...
    if (img2 != null) {
      await prefs.setString('img2Path', img2!.path);
    }
    if (img3 != null) {
  ...
  void offlinehandleSubmit() async {
    ...
    Map<String, dynamic> visitrow = {
      ...
      db.service_provider : utility.service_provider_c
    };
  ...
  Future<void> fetchVehicleNumbers() async {
    ...
    try {
      ...
      print('responsejfkf${response.statusCode}');
  ...
  String OdometerImageBase64() {
    if (img3 != null) {
      List<int> imageBytes = img3!.readAsBytesSync();
      return base64Encode(imageBytes);
    }
    return '';
  }
        ...
        showAnimatedDialog(
          ...
          builder: (BuildContext context) {
            return AlertDialog(
              ...
              content: const Text("Charging data uploaded successfully."),
              actions: [
                TextButton(
                  onPressed: () {
                    Navigator.of(context).pop();
  ...
  //         'endsocimage',
  //         img1!.path,
  //         contentType: MediaType.parse(lookupMimeType(img1!.path) ?? 'application/octet-stream'),
  //       ));
  //     }
  //     if (img2 != null) {
  //       String fileName = img2!.path.split('/').last;
  ...
  //         'startsocimage',
  //         img2!.path,
  //         contentType: MediaType.parse(lookupMimeType(img2!.path) ?? 'application/octet-stream'),
  //       ));
  //     }
  //     if (img3 != null) {
  //       String fileName = img3!.path.split('/').last;
  ...
  //         'odometerimage',
  //         img3!.path,
  //         contentType: MediaType.parse(lookupMimeType(img3!.path) ?? 'application/octet-stream'),
  //       ));
  //     }
  //
  //     request.headers.addAll({
  ...
  //               ],
  //             ),
  //             content: Text("Failed to upload charging data. Status code: ${response.statusCode}"),
  //             actions: [
  //               TextButton(
  //                 onPressed: () {
  //                   Navigator.of(context).pop();
                                              ...
                                              Padding(
                                                ...
                                                child: ElevatedButton(
                                                  onPressed: () {
                                                    ...
                                                    //For  image optinal so that i hide this code
                                                    // if (img3 == null) {
                                                    //   Fluttertoast.showToast(
                                                    //       msg:
